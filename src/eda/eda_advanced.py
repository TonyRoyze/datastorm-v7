...
oob_mask = np.invert(
    df["Latitude"].between(SL_LAT_MIN, SL_LAT_MAX)
    & df["Longitude"].between(SL_LON_MIN, SL_LON_MAX)
)
n_oob = oob_mask.sum()
...
for c in range(8):
    ...
    ax.scatter(
        df_valid.loc[m, "Longitude"],
        df_valid.loc[m, "Latitude"],
        s=2,
        alpha=0.5,
        label=f"C{c} ({m.sum()})",
    )
...
ax = axes[1]
order = (
    df_valid.groupby("spatial_cluster")["total_historic_volume"]
    .median()
    .sort_values()
    .index
)
...
df_valid["lat_bin"] = pd.cut(
    df_valid["Latitude"], bins=lat_bins, labels=False, include_lowest=True
)
df_valid["lon_bin"] = pd.cut(
    df_valid["Longitude"], bins=lon_bins, labels=False, include_lowest=True
)
...
fig.tight_layout()
fig.savefig(
    os.path.join(PLOT_DIR, "turing_rd_inputs.png"), dpi=150, bbox_inches="tight"
)
plt.close()
...
fig.tight_layout()
fig.savefig(
    os.path.join(PLOT_DIR, "laplacian_fields.png"), dpi=150, bbox_inches="tight"
)
plt.close()
print("  → laplacian_fields.png")

cell_stats = (
    df_valid.groupby(["lat_bin", "lon_bin"])
    .agg(
...
axes[0, 0].scatter(
    cell_stats["n_outlets"], cell_stats["total_vol"], s=8, alpha=0.4, c="#e63946"
)
axes[0, 0].set_xlabel("Outlets per Cell")
axes[0, 0].set_ylabel("Total Volume per Cell")
axes[0, 0].set_title("Outlet Density vs Demand (per cell)")
axes[0, 1].scatter(
    cell_stats["n_outlets"], cell_stats["avg_vol"], s=8, alpha=0.4, c="#457b9d"
)
axes[0, 1].set_xlabel("Outlets per Cell")
axes[0, 1].set_ylabel("Avg Volume per Outlet")
axes[0, 1].set_title("Density vs Per-Outlet Demand")
axes[1, 0].scatter(
    cell_stats["avg_coolers"], cell_stats["avg_vol"], s=8, alpha=0.4, c="#2a9d8f"
)
axes[1, 0].set_xlabel("Avg Coolers per Outlet")
axes[1, 0].set_ylabel("Avg Volume per Outlet")
axes[1, 0].set_title("Cooler Capacity vs Demand")
axes[1, 1].scatter(
    cell_stats["n_outlets"], cell_stats["avg_tx_size"], s=8, alpha=0.4, c="#e9c46a"
)
axes[1, 1].set_xlabel("Outlets per Cell")
axes[1, 1].set_ylabel("Avg Transaction Size")
...
np.savez(
    ...
    lon_bins=lon_bins,
    cell_size=CELL_SIZE,
    grid_shape=np.array([n_lat, n_lon]),
)

df_valid["cell_demand_A"] = df_valid.apply(
    lambda r: (
        A_norm[int(r["lat_bin"]), int(r["lon_bin"])]
        if pd.notna(r["lat_bin"]) and pd.notna(r["lon_bin"])
        else 0
    ),
    axis=1,
)
df_valid["cell_density_B"] = df_valid.apply(
    lambda r: (
        B_norm[int(r["lat_bin"]), int(r["lon_bin"])]
        if pd.notna(r["lat_bin"]) and pd.notna(r["lon_bin"])
        else 0
    ),
    axis=1,
...
df_valid["cell_intensity"] = df_valid.apply(
    lambda r: (
        intensity_norm[int(r["lat_bin"]), int(r["lon_bin"])]
        if pd.notna(r["lat_bin"]) and pd.notna(r["lon_bin"])
        else 0
    ),
    axis=1,
...
df_valid["laplacian_A"] = df_valid.apply(
    lambda r: (
        lap_A[int(r["lat_bin"]), int(r["lon_bin"])]
        if pd.notna(r["lat_bin"]) and pd.notna(r["lon_bin"])
        else 0
    ),
    axis=1,
