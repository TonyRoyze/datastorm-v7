...
tx_m = (
    tx.groupby(["Year", "Month"])
    .agg(total_vol=("Volume_Liters", "sum"), n_outlets=("Outlet_ID", "nunique"))
    .reset_index()
)
...
axes[0].hist(
    df_valid["total_historic_volume"],
    bins=80,
    color="#5e60ce",
    edgecolor="white",
    linewidth=0.3,
...
axes[1].hist(
    np.log1p(df_valid["total_historic_volume"]),
    bins=80,
    color="#5e60ce",
    edgecolor="white",
    linewidth=0.3,
...
axes[0].hist(
    df_valid["Cooler_Count"].clip(0, 20),
    bins=20,
    color="#2a9d8f",
    edgecolor="white",
    linewidth=0.3,
...
axes[1].hist(
    np.log1p(df_valid["transaction_count"]),
    bins=60,
    color="#e9c46a",
    edgecolor="white",
    linewidth=0.3,
...
axes[2].hist(
    np.log1p(df_valid["avg_transaction_size"]),
    bins=60,
    color="#f4a261",
    edgecolor="white",
    linewidth=0.3,
...
ax.barh(
    poi_means.index, poi_means.values, color="#6d597a", edgecolor="white", linewidth=0.5
)
