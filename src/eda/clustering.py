...
PLOT_DIR = os.path.join(BASE, "outputs", "clustering")
...
print("[2/5] GPS forensics & filtering...")
...
print("[4/5] Turing RD grid computation...")
CELL_SIZE = 0.01
...
A_norm = A_grid / (A_grid.max() if A_grid.max() > 0 else 1)
B_norm = B_grid / (B_grid.max() if B_grid.max() > 0 else 1)
...
fig.suptitle("Turing RD Input Variables", fontsize=14, y=1.01)
fig.tight_layout()
...
print(f"Cluster sizes:\n{df_valid['spatial_cluster'].value_counts().sort_index().to_string()}")
print("Done!")
