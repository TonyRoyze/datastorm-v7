#!/usr/bin/env python3
"""
Latent Demand Estimation Modeling Pipeline
=====================================================
Implements:
...
oob = np.invert(df["Latitude"].between(5.9, 9.9) & df["Longitude"].between(79.5, 81.9))
...
tx_outlet = (
    tx.groupby(["Outlet_ID", "Year", "Month"])
    .agg(
...
outlet_ts["volume_cv"] = outlet_ts["std_monthly_volume"] / (
    outlet_ts["avg_monthly_volume"] + 1e-8
)
...
flatline_flags = (
    tx_outlet_sorted.groupby("Outlet_ID").apply(detect_flatline).reset_index()
)
flatline_flags.columns = ["Outlet_ID", "flatline_flag"]

round_flags = (
    tx_outlet_sorted.groupby("Outlet_ID").apply(detect_round_numbers).reset_index()
)
round_flags.columns = ["Outlet_ID", "round_number_flag"]

# Merge both signals
flatline_flags = flatline_flags.merge(round_flags, on="Outlet_ID", how="left")
flatline_flags["censoring_signal"] = (
    (flatline_flags["flatline_flag"] == 1) | (flatline_flags["round_number_flag"] == 1)
).astype(int)
...
dist_map = tx.groupby("Outlet_ID")["Distributor_ID"].agg(
...
jan_seas["seas_score"] = (
    jan_seas["Seasonality_Index"]
    .map({"Favorable": 1.2, "Moderate": 1.0, "Un-Favorable": 0.8})
    .fillna(1.0)
)
...
if has_turing:
    assert turing_feats is not None
...
df["distributor_enc"] = le_dist.fit_transform(
    df["primary_distributor"].fillna("Unknown")
)
...
cv_norm = df["volume_cv"].clip(0, 2).to_numpy()
flatline = df["flatline_flag"].to_numpy()
round_num = df["round_number_flag"].to_numpy()
censor_sig = df["censoring_signal"].to_numpy()
size_enc = df["outlet_size_enc"].to_numpy()
df["constraint_score"] = (
    (1.0 - cv_norm / 2) * 0.3
    + flatline * 0.25
    + round_num * 0.15
...
unconstrained_mask = np.invert(is_censored)
...
for feat, imp in feat_imp.head(6).items():
    print(f"    {feat}: {imp:.3f}")
print(
    f"  Tobit predictions  mean: {y_tobit.mean():.1f}, ",
...
print(
    f"  Peer 90th percentile  mean: {peer_90th.mean():.1f}, max: {peer_90th.max():.1f}"
)
...
for size in df["Outlet_Size"].unique():
    for otype in df["Outlet_Type"].unique():
        ...
        cooler_factor = df.loc[specific_mask, "Cooler_Count"].values / (
            df.loc[specific_mask, "Cooler_Count"].max() + 1e-8
        )
...
raw_ensemble = y_tobit * W_TOBIT + sfa_frontier * W_SFA + peer_90th * W_PEER
...
n_nulls = pd.isna(output_df["Maximum_Monthly_Liters"]).sum()
...
ax = axes[0, 0]
ax.scatter(
    df["avg_monthly_volume"], df["Maximum_Monthly_Liters"], s=2, alpha=0.3, c="#e63946"
)
...
sc0 = axes[0].scatter(
    df["Longitude"],
    df["Latitude"],
    c=np.log1p(df["avg_monthly_volume"]),
    cmap="magma",
    s=2,
    alpha=0.5,
)
