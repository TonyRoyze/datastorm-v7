...
A[np.invert(occupied)] = 0.0
B[np.invert(occupied)] = 0.0
...
for t in range(T):
    ...
    A[np.invert(land_mask)] = 0
    B[np.invert(land_mask)] = 0
...
# Map back to outlets
outlet_feats = pd.read_parquet(
    os.path.join(BASE, "data/gold/turing_outlet_features.parquet")
)
...
im0 = axes[0].imshow(
    A_init, origin="lower", cmap="inferno", aspect="auto", extent=extent
)
axes[0].set_title("Initial Demand (A₀)")
plt.colorbar(im0, ax=axes[0], shrink=0.6)

im1 = axes[1].imshow(
    A_steady, origin="lower", cmap="inferno", aspect="auto", extent=extent
)
axes[1].set_title(f"Steady-State Demand (A, T={T})")
...
fig.tight_layout()
fig.savefig(
    os.path.join(PLOT_DIR, "09_rd_simulation.png"), dpi=150, bbox_inches="tight"
)
plt.close()
